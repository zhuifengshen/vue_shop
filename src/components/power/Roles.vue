<template>
  <div>
    ...
    <!-- 分配权限的对话框 -->
    <el-dialog title="权限分配" :visible.sync="setRightDialogVisible" width="50%" @close="setRightDialogClose">
      <!-- 树形控件 -->
      <el-tree
        :data="rightslist"
        :props="treeProps"
...
export default {
  ...
  methods: {
    ...
    async removeRightById(role, rightId) {
      ...
      // 直接根据返回数据更新当前该角色权限即可，避免重新刷新所有数据
    ...
    async showSetRightDialog(role) {
      ...
      // 把获取到的权限数据保存到 data 中的 rightslist
    ...
    // 通过递归的方式获取角色下所有三级权限的 ID，并保存到 defKeys 数组中
    ...
    async showEditRoleDialog(id) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('根据ID查询角色信息失败！')
      }
